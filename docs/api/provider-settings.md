...
The response may contain `version`, `email`, `whatsappNumber`, `officeAddress`, `website`, and `availableActions`. `whatsappNumber` is contact data, not an authentication identifier. The response does not contain user IDs, provider application IDs, legal or trade data, permissions, audit data, storage keys, secrets, or private document URLs.
