...
- `POST /api/v1/auth/register/seeker` consumes that authority exactly once, creates a verified seeker account from the phone-and-email pair bound to the email-delivered OTP, and returns the shared access-token session shape. The rotating refresh token is set only as the `HttpOnly` `sadat_refresh` cookie.
