...
- `POST /api/v1/auth/register/seeker` consumes that authority exactly once, creates a verified seeker account from the normalized email bound to the email-delivered OTP, and returns the shared access-token session shape. The rotating refresh token is set only as the `HttpOnly` `sadat_refresh` cookie.
- `GET/PATCH /api/v1/me` expose only the authenticated seeker projection. The email and role are server-owned; update requests allow only `firstName`, `lastName`, and `locale`. Phone is not part of Seeker identity or this projection.
