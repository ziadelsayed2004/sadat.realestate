...
## 13 September: PRV-06 official-source reconciliation

- Read official desktop node `6017:20034` and mobile node `6017:118284` directly. The pricing route now uses the shared add-property heading, places the eight-step rail after it with step 4 active, and highlights Add property in the desktop provider rail.
- Preserved the exact `propertyPricingStepSchema` flow for price/currency and the optional localized payment plan. The reviewed browser fixture now renders the safe authenticated commission projection with source, kind, value, effective date, and policy version instead of an unavailable-state placeholder.
- Negotiability, payment-method selection, maintenance deposit, additional fees, payment notes, provider account type, policy-detail link, and acknowledgement are absent from the active contracts and were not invented as controls.
- Desktop baseline update passed 2/2. Normal verification passed 6/6 across AR/EN Desktop/Tablet/Mobile, including no-overflow geometry at 1024×1340 and 402×1514 and five distinct commission records. Build/budgets, translation check, typecheck, focused lint, and diff checks passed. PRV-06 remains `PARTIAL_EXTERNAL`; strict totals stay 91/119 with 28 open. No Production launch or Demo purge ran.

## 13 September: PRV-07 official-source reconciliation

- Read official desktop node `6017:20229` and mobile node `6017:118435` directly. The feature/service route now uses the shared add-property heading, places the eight-step rail after it with step 5 active, highlights Add property on desktop, and uses the compact source geometry with full-width actions on mobile.
...
## 13 سبتمبر: ضمان rollback لمراجعة مقدم العقار

- وُسع فحص ضمانات GUIDE-11/12/13 بحقن فشل حقيقي بعد إدراج سجل audit داخل transaction مراجعة مقدم العقار. أثبت MongoDB أن حالة الحساب والملف والطلب، وإلغاء الجلسة، وانتقال الحالة، وسجل audit كلها عادت كما كانت؛ ثم نجحت إعادة القرار نفسه بانتقال واحد وسجل audit واحد.
