...
### SEO downstream cycle

- The SEO namespace now has an anonymous, read-only `GET /api/v1/public/settings/seo` projection. It returns only complete validated localized title/description, canonical origin, robots policy, sitemap status, and optional title separator/Google verification; version, actor, audit, and unrelated Admin fields are excluded.
- Public SSR consumes the projection with a 1.5-second fail-safe timeout. It applies homepage defaults, appends the configured site title to internal page titles when a separator exists, renders the verification meta tag, uses the configured canonical origin, and forces `noindex` across public pages when indexing is disabled.
- `robots.txt` and `sitemap.xml` consume the same projection. Disabled indexing emits `Disallow: /`; an inactive sitemap is not advertised and returns 404. Missing or invalid settings retain the existing safe static behavior.
- Verification passed 10/10 focused API tests, including the pre-existing SEO/privacy schema suite, and 10/10 Web SEO/SSR/Production-preview tests. The preview test starts a real mock API and the built Production server, then checks rendered HTML, canonical metadata, verification, robots, and sitemap behavior. Runtime/policy inventory is 188/188; API audit, OpenAPI, Postman, lint, TypeScript, and Production build pass.
- Privacy, display/population, property, advertising, and moderation policy consumers remain open; this SEO closure does not imply those namespaces are enforced yet.
