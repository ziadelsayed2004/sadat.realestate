...
- A read-only Production audit authenticated the supplied account as a real `admin` account and exercised all 51 sidebar routes on Desktop and Pixel 5. Pixel 5 width matched at 393px on every route, and the property APIs plus public property list/detail returned 200 with `installmentAvailable`. The latest candidate is pushed to `main` as `0644915` (including `67c0e71`, `87514c7`, and `95da359`); deployment remains outstanding because this workstation has no VPS SSH key or authenticated Hostinger control session. GitHub CI could not start its runner because of an account billing lock, so it produced no code-test result. Production still serves the pre-change asset hashes, and the old article query contract returns 400 until deployment.
