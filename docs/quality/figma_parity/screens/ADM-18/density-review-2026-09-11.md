...
On 12 September, the row action was aligned to the recovered source at `96×28px`, `14px` radius and `12/16px` semibold type. The ADM-18-only column distribution now reserves enough inline space for the source-sized action; a geometry assertion proves the button remains fully contained in both AR and EN. Reviewed visual baselines also record the current compact six-row fixture and the active sidebar item remaining visible on the viewing/issues routes. The focused component suite passed 6/6, the AR/EN request and visual suites passed 12/12 without snapshot-update mode, and the Web build/bundle budget passed.

This is **not full Figma closure**. Remaining differences include exact CSV placement/styling, exact toolbar width/height/spacing, badge styling and date formatting, and a directly comparable source/runtime populated state. The right-hand Arabic navigation remains the approved product exception. Current captures contain deterministic QA data that differs from the source content, so pixel differences must not be interpreted as a like-for-like parity score. The official 90/119 count remains unchanged.
