...
- Admin settings, advertising, notifications, and audit content now start at the measured 24px inset. The focused layout/settings run passed 16 tests and skipped 20 desktop-scoped cases. The advertising/notifications/filter/sidebar run passed 66, failed two stale 72px assertions, and skipped 28; the corrected sidebar rerun passed all 30 tests. Skipped cases are not passed tests.
...
## Admin settings controls recheck

- Direct Figma contexts for ADM-53/55/56/57/58 were used to implement the previously absent settings controls: property publication and media limits; advertising placements, dimensions, formats, quote validity, and proof methods; SEO defaults and indexing; privacy/security switches; and display, population, and community moderation defaults.
- The forms use localized text areas, bounded numeric inputs, select controls for enumerated policies, full-row boolean controls, and full-row arrays. They support a namespace that returns `SETTINGS_NOT_FOUND` by creating it with `expectedVersion: 0`, while preserving unknown stored keys during later edits.
- The save path continues through the protected, versioned, reason-required Admin settings API and its audit write. The focused component suite passed 15/15; lint and TypeScript passed. After stabilizing font loading and form line-height, the implementation visual baseline and a subsequent normal Arabic/English run both passed 2/2.
- This closes the missing-control and generic persistence gap only. Public metadata generation and runtime enforcement of the saved property, privacy, advertising, display, and moderation values remain unverified downstream consumers and must not be reported as complete cycles yet.

## SEO settings runtime closure

- Added the anonymous `GET /api/v1/public/settings/seo` safe projection and documented it in the implemented blueprint, OpenAPI, Postman, and API guide. The projection is emitted only when all required SEO values are valid and strips Admin version, actor, audit, and unrelated values.
- The built SSR server consumes that projection for localized homepage title/description, canonical origin, internal-page title separator, Google site verification, and a global noindex override. The crawler endpoints consume the same values: noindex becomes `Disallow: /`, and an inactive sitemap is omitted from robots and returns 404.
- Missing, incomplete, invalid, or unavailable settings fail back to the existing static SEO and crawler behavior within a bounded timeout. Focused verification passed 10/10 API tests, including the older SEO/privacy contract tests, and 10/10 Web SEO/SSR/Production-preview tests; build, lint, TypeScript, API audit (188 runtime/policy routes), OpenAPI, and Postman are green.
