...
- تعريفات الأدوار والإعدادات والمواقع والتصنيفات الحقيقية وفق القائمة الصريحة في `apps/api/src/modules/database/production-launch-policy.ts`. تُحذف منها السجلات الموسومة `synthetic=true` أو التي تحمل `seedKey`.
- تُمسح إسنادات الأدوار القديمة؛ صلاحية Super Admin تبقى من bootstrap الذي يتحقق منه عقد RBAC الفعلي.
- بنية مجموعات MongoDB وفهارسها؛ العملية تستخدم `deleteMany` داخل transaction ولا تستخدم drop.

تُمسح السجلات التشغيلية المصنفة صراحة، بما فيها الجلسات وOTP والحسابات والعقارات والمشروعات والإعلانات
...
أي مجموعة غير مدرجة في السياسة توقف الخطة والتنفيذ قبل الحذف حتى تُراجع. وتتوقف العملية أيضًا إذا كان موقع أو تصنيف حقيقي سيفقد مرجعه الأب. معرفات منشئ ومحدث السجلات المرجعية تبقى مراجع تاريخية ولا تُنشئ حسابات جديدة.
...
الأمر `npm run production:launch:purge` يعرض الخطة فقط. التنفيذ المباشر يتطلب `-- --apply` مع متغيرات التأكيد والنسخة الاحتياطية. متغير البيئة المعتمد في المشروع هو `APP_ENV=production`، وهو المقابل الفعلي لشرط بيئة Production.

اختبار MongoDB المحلي المعزول: `node --env-file=.env.local --import tsx scripts/verify-production-launch-local.mjs`. ينشئ قاعدة اختبار فريدة على loopback وينظف مجموعاتها بعد الاختبار؛ لا يتصل بقاعدة Production. دليله في `docs/quality/guide-runs/production-launch-local-latest.json` ولا يثبت تنفيذ الإطلاق الحقيقي.
