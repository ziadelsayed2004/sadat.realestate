...
- اختبارات Web الكاملة نجحت: 65 ملف Vitest و376 اختبارًا، ثم 77 اختبار Node إضافيًا.
- تدقيق العقود نجح: 183 مسار runtime مقابل 183 policy route، وOpenAPI وPostman صالحان.
- تم توحيد زر المقارنة على الهاتف مع حد لمس 44px وحالة اختيار واضحة، والهيدر العام
  يعرض زر قائمة بثلاث شرطات عند عرض الهاتف مع دعم Escape وخصائص ARIA.
- تم ربط ملخص الباحث ببيانات الطلبات والمعاينات والإشعارات الفعلية مع إسقاط آمن؛ لا تُعرض
  ملاحظات داخلية أو بيانات اعتماد في لوحة الباحث.
...
- مصدر الحالة الآلي الأحدث هو
  `docs/quality/figma_parity/CURRENT_COMPLETION_AUDIT.json`: من أصل 119 شاشة
  قابلة للتدقيق (Auth 19، Seeker 10، Provider 24، Admin 66)، أُغلقت 90 وبقيت 29
  `PARTIAL_EXTERNAL` أو `BLOCKED_SOURCE`. لا نعلن تطابقًا بصريًا كاملًا قبل توفير
  مصادر/بيانات Figma الخارجية المطلوبة.
...
## تحديث قرار عوائق التصميم — 2026-09-13

- السجل السابق الذي كان يعرض `91/119` و28 شاشة مفتوحة أصبح تاريخيًا. بعد قرار مالك المشروع المسجل في `docs/quality/figma_parity/OWNER_BLOCKER_RESOLUTION_2026-09-13.json` أصبحت بوابة Figma الصارمة `119/119` وصفر مفتوح.
- لا يدعي الإغلاق استعادة إطار Figma تاريخي لـ `ADM-54`؛ المصدر المحلي الذي أنشأه المالك هو المصدر الحالي المعتمد، ونجحت له baselines العربية والإنجليزية.
- هذا القرار لا يغير بوابة التشغيل الإنتاجي: المشروع باقٍ في وضع Demo، وما زالت خطوات الخادم وTLS وSMTP والنسخ والاستعادة والدخان الإنتاجي تتطلب تنفيذًا صريحًا منفصلًا.
