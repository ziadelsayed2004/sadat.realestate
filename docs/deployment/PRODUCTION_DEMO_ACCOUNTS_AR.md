...
## دورة حياة نسخة Demo الحالية

نسخة Production الحالية مخصصة للعرض مؤقتًا بقرار المالك. استخدم وضع `demo` عند نشر نسخة تحتاج استكمال بيانات العرض؛ هذا الوضع يضيف السجلات الاصطناعية الناقصة ولا يحذف السجلات الحقيقية:
...
استخدم `update` للتحديثات التي لا تحتاج تغيير بيانات العرض. لا تستخدم `empty` أو `launch` أثناء مرحلة Demo الحالية. عندما يطلب المالك إزالة العرض لاحقًا، يزيل `empty` السجلات الموسومة synthetic فقط، بينما ينفذ `launch` الانتقال النهائي المحمي بعد نسخة احتياطية مع الاحتفاظ بحساب Super Admin الحقيقي المحدد. تفاصيل `launch` موجودة في `REAL_PRODUCTION_LAUNCH_AR.md`.
