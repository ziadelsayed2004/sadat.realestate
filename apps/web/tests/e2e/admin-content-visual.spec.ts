...
test('ADM-25 and ADM-26 match the approved desktop visual baselines', async ({ page }) => {
  ...
  await page.route('**/api/v1/admin/articles**', async route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { items: [{ id: 'aaaaaaaaaaaaaaaaaaaaaaaa', categoryId: 'bbbbbbbbbbbbbbbbbbbbbbbb', slug: 'buying-in-sadat', title: { ar: 'دليل الشراء', en: 'Buying in Sadat', 'zh-CN': '在萨达特购买' }, body: { ar: 'محتوى المقال', en: 'Article body', 'zh-CN': '文章内容' }, authorId: 'cccccccccccccccccccccccc', status: 'draft', version: 3, createdAt: '2026-08-10T10:00:00.000Z', updatedAt: '2026-08-18T10:00:00.000Z', availableActions: ['update', 'submit'] }] }, meta: { requestId: 'admin-content-visual-list', page: 1, limit: 20, total: 1 } }) }));
  await page.route('**/api/v1/admin/article-categories**', async route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { items: [{ id: 'bbbbbbbbbbbbbbbbbbbbbbbb', slug: 'buying-tips', name: { ar: 'نصائح الشراء', en: 'Buying tips', 'zh-CN': '购买建议' }, description: { ar: 'إرشادات', en: 'Guides', 'zh-CN': '指南' }, displayOrder: 1, active: true, version: 2, createdAt: '2026-08-10T10:00:00.000Z', updatedAt: '2026-08-18T10:00:00.000Z', availableActions: ['update', 'delete'] }] }, meta: { requestId: 'admin-content-visual-categories', page: 1, limit: 20, total: 1 } }) }));
