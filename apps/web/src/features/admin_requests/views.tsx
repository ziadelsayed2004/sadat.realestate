...
import './styles.css';
import { RequestExport } from './request-export.tsx';
...
function dateLabel(value: string | undefined, locale: SupportedLocale, dateOnly = false): string {
  if (value === undefined) return '—';
  try {
    if (dateOnly) return new Intl.DateTimeFormat(locale === 'ar' ? 'ar-EG-u-nu-latn' : locale, { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(value));
...
function requestCustomerLabel(request: RequestData, locale: SupportedLocale): string {
  const localized = requestPayloadText(request, locale === 'ar' ? ['customerNameAr'] : ['customerNameEn']);
  if (localized !== undefined) return localized;
  const explicit = requestPayloadText(request, ['customerName', 'fullName', 'name']);
  if (explicit !== undefined) return explicit;
  const parts = [requestPayloadText(request, ['firstName']), requestPayloadText(request, ['lastName'])].filter((value): value is string => value !== undefined);
  if (parts.length > 0) return parts.join(' ');
  const email = requestPayloadText(request, ['email']);
  if (email !== undefined) return email;
  const reference = request.seekerId ?? request.providerId ?? request.creatorId;
  return reference === undefined ? copyUnavailable(locale) : `…${reference.slice(-6)}`;
}
...
function requestPropertyLabel(request: RequestData, locale: SupportedLocale): string {
  const localized = request.property?.name?.[locale] ?? request.property?.name?.en ?? request.property?.name?.ar;
...
function requestSourceLabel(request: RequestData, locale: SupportedLocale): string {
  const localized = requestPayloadText(request, locale === 'ar' ? ['sourceLabelAr'] : ['sourceLabelEn']);
  if (localized !== undefined) return localized;
...
function requestAssigneeLabel(request: RequestData, locale: SupportedLocale): string {
  const payloadLabel = requestPayloadText(request, locale === 'ar' ? ['assigneeNameAr', 'assigneeName', 'responsibleName'] : ['assigneeNameEn', 'assigneeName', 'responsibleName']);
  if (payloadLabel !== undefined) return payloadLabel;
  return request.assignedTo === undefined ? (locale === 'ar' ? 'غير مسند' : 'Unassigned') : `…${request.assignedTo.slice(-6)}`;
}

function RequestTable({ copy, data, locale, onSelect, overdue = false, sourceDensity = false }: { readonly copy: AdminRequestsCopy; readonly data: RequestListData | OverdueRequestListData; readonly locale: SupportedLocale; readonly onSelect: (request: RequestData) => void; readonly overdue?: boolean; readonly sourceDensity?: boolean }) {
...
function Filters({ copy, locale, screen, query, onApply, onClear }: { readonly copy: AdminRequestsCopy; readonly locale: SupportedLocale; readonly screen: AdminRequestsScreen; readonly query: Partial<RequestListQuery>; readonly onApply: (query: Partial<RequestListQuery>) => void; readonly onClear: () => void }) {
  ...
  const apply = (nextStatus = status) => onApply({ page: 1, ...(search.trim() === '' ? {} : { search: search.trim() }), ...(nextStatus === '' ? {} : { status: nextStatus }), ...(type === '' ? {} : { type }) });
  const statusField = <div><label htmlFor="admin-requests-status">{copy.status}</label><select id="admin-requests-status" value={status} onChange={event => setStatus(event.target.value as RequestStatus | '')}><option value="">{copy.allStatuses}</option>{requestStatuses.map(item => <option key={item} value={item}>{copy.statusLabel[item]}</option>)}</select></div>;
  const actions = <div className="admin-requests__filter-actions"><Button type="submit">{copy.apply}</Button><Button type="button" variant="secondary" onClick={() => { setSearch(''); setStatus(''); setType(''); onClear(); }}>{copy.clear}</Button></div>;
  return <form className={`admin-requests__filters${screen === 'all' ? ' admin-requests__filters--compact' : ''}`} role="search" aria-label={copy.filters} onSubmit={event => { event.preventDefault(); apply(); }}>
    {screen === 'all' ? <div className="admin-requests__quick-status" role="group" aria-label={copy.status}>
      {(['', 'new', 'under_review', 'contacted', 'closed'] as const).map(value => <button key={value || 'all'} type="button" aria-pressed={status === value} onClick={() => { setStatus(value); apply(value); }}>{value === '' ? requestMetricLabel(locale, 'all') : copy.statusLabel[value]}</button>)}
    </div> : null}
    <div className="admin-requests__search-field"><label htmlFor="admin-requests-search">{copy.search}</label><input id="admin-requests-search" type="search" value={search} onChange={event => setSearch(event.target.value)} placeholder={copy.searchPlaceholder} /></div>
    {screen === 'all' ? <details className="admin-requests__advanced-filters"><summary>{copy.filters}</summary><div className="admin-requests__advanced-fields">
      {statusField}
      <div><label htmlFor="admin-requests-type">{copy.type}</label><select id="admin-requests-type" value={type} onChange={event => setType(event.target.value as RequestType | '')}><option value="">{copy.allTypes}</option>{requestTypes.map(item => <option key={item} value={item}>{copy.typeLabel[item]}</option>)}</select></div>
      {actions}
    </div></details> : <>{statusField}{actions}</>}
  </form>;
}
...
export function AdminRequests({ locale, session, authClient, apiOrigin, initialRequests, initialOverdue, initialViewings, initialIssues, loadRequests, loadOverdue, loadViewings, loadIssues, loadRequest, transition, assign, note, resolveIssue }: AdminRequestsProps) {
  const baseCopy = getAdminRequestsCopy(locale);
  ...
  const copy: AdminRequestsCopy = projection.screen === 'all' && locale === 'ar' ? {
    ...baseCopy,
    titles: { ...baseCopy.titles, all: 'إدارة الطلبات' },
    searchPlaceholder: 'بحث بالاسم أو الهاتف...',
    view: 'عرض التفاصيل',
    created: 'التاريخ',
    count: count => `${count.toLocaleString('ar-EG-u-nu-latn')} طلب`,
    statusLabel: { ...baseCopy.statusLabel, under_review: 'قيد المتابعة' }
  } : baseCopy;
  ...
  function applyFilters(next: Partial<RequestListQuery>): void { setQuery({ page: 1, limit: 20, ...next, ...(projection.fixedType === undefined ? {} : { type: projection.fixedType }) }); setAttempt(value => value + 1); setSelectedRequest(undefined); }
