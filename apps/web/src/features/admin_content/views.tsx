...
function CmsStatePanel({ state, locale, onRetry, form }: { readonly state: AdminCmsState; readonly locale: SupportedLocale; readonly onRetry: () => void; readonly form: boolean }) {
  ...
  return <section className="admin-content__state" data-state={state} aria-label={message.title}><StateMessage state={state} title={message.title} message={message.body} loadingVariant={form ? 'form' : 'list'} onRetry={state === 'retry' ? onRetry : undefined} retryLabel={copy.retry} />{state === 'error' ? <Button type="button" variant="secondary" onClick={onRetry}>{copy.retry}</Button> : null}</section>;
}
