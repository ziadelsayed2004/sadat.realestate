...
function StatePanel({ state, locale, onRetry, form }: { readonly state: Exclude<AdminAdsState, 'success' | 'empty' | 'not_found'>; readonly locale: SupportedLocale; readonly onRetry: () => void; readonly form: boolean }) {
  const copy = getAdminAdsCopy(locale);
  const message = copy.states[state];
  return <section className="admin-ads__state" data-state={state} aria-label={message.title}><StateMessage state={state} title={message.title} message={message.body} loadingVariant={form ? 'form' : 'table'} retryLabel={copy.next} onRetry={state === 'retry' ? onRetry : undefined} />{state === 'error' ? <Button variant="secondary" size="sm" onClick={onRetry}>{copy.next}</Button> : null}</section>;
}
...
function RequestsTable({ data, locale, onDetail }: { readonly data: AdAdminRequestListData; readonly locale: SupportedLocale; readonly onDetail: (id: string) => void }) {
  ...
  const labels = copy.requestMetrics;
...
function QuoteIssuePanel({ locale, onIssue }: { readonly locale: SupportedLocale; readonly onIssue: (input: { currency: string; lineItems: [{ description: string; quantity: number; unitAmountMinor: number }]; validUntil: string; terms: string }) => Promise<void> }) {
  ...
  const quoteCopy = getAdminAdsCopy(locale).quoteIssue;
  return <form className="admin-ads__review-card" onSubmit={submit}><h3>{quoteCopy.title}</h3><p>{quoteCopy.help}</p><fieldset disabled={saving}><label className="admin-ads__field" htmlFor="admin-ad-quote-description">{quoteCopy.description}<input id="admin-ad-quote-description" value={description} onChange={event => setDescription(event.target.value)} minLength={2} maxLength={300} required /></label><label className="admin-ads__field" htmlFor="admin-ad-quote-amount">{quoteCopy.amount}<input id="admin-ad-quote-amount" type="number" min="0" step="1" value={amount} onChange={event => setAmount(event.target.value)} required /></label><label className="admin-ads__field" htmlFor="admin-ad-quote-valid">{quoteCopy.validUntil}<input id="admin-ad-quote-valid" type="datetime-local" value={validUntil} onChange={event => setValidUntil(event.target.value)} required /></label><label className="admin-ads__field" htmlFor="admin-ad-quote-terms">{quoteCopy.terms}<textarea id="admin-ad-quote-terms" value={terms} onChange={event => setTerms(event.target.value)} minLength={2} maxLength={2000} required /></label><Button type="submit" loading={saving}>{quoteCopy.submit}</Button></fieldset>{error ? <p role="alert">{quoteCopy.error}</p> : null}</form>;
}
...
function PaymentProofTable({ data, locale, review, onReview }: { readonly data: PaymentProofAdminListData; readonly locale: SupportedLocale; readonly review: boolean; readonly onReview: (id: string) => void }) {
  ...
  const labels = copy.proofMetrics;
...
function FinancialTable({ data, locale, onDetail }: { readonly data: AdFinancialReviewListData; readonly locale: SupportedLocale; readonly onDetail: (id: string) => void }) {
  ...
  const labels = copy.financialMetrics;
  const metrics: readonly AdsMetric[] = [
...
export function AdminAds({ locale, session, authClient, apiOrigin, url, loadRequests, loadRequestDetail, reviewRequest, issueQuote, loadPaymentProofs, reviewPaymentProof, loadCalendar, loadFinancialReview, loadFinancialDetail, loadLedger }: AdminAdsProps) {
  ...
  return <section className="admin-ads" data-screen-id={projection.screenId} data-route={projection.route} data-device-scope="desktop" data-admin-ads-state={state}>
    ...
    <div className="admin-ads__content">
      ...
      {state === 'loading' || state === 'error' || state === 'retry' || state === 'permission' ? <StatePanel state={state} locale={locale} form={((projection.view === 'requests' || projection.view === 'financial') && selectedRequestId !== undefined) || projection.view === 'review'} onRetry={() => setAttempt(value => value + 1)} /> : null}
