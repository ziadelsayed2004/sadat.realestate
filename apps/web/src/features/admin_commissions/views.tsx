...
function StatePanel({ state, locale, onRetry, form }: { readonly state: Exclude<AdminCommissionsState, 'success' | 'empty' | 'not_found'>; readonly locale: SupportedLocale; readonly onRetry: () => void; readonly form: boolean }) {
  ...
  return (
    <section className="admin-commissions__state" data-state={state} aria-label={message.title}>
      <StateMessage state={state === 'permission' ? 'permission' : state === 'loading' ? 'loading' : state === 'retry' ? 'retry' : 'error'} title={message.title} message={message.body} loadingVariant={form ? 'form' : 'table'} retryLabel={copy.actions.retry} onRetry={state === 'retry' ? onRetry : undefined} />
