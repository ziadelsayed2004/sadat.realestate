...
type AdminRbacView = { readonly kind: "users"; readonly path: string } | { readonly kind: "user-create"; readonly path: string } | { readonly kind: "user-detail"; readonly path: string; readonly id: string } | { readonly kind: "roles"; readonly path: string } | { readonly kind: "role-detail"; readonly path: string; readonly id: string } | { readonly kind: "not-found"; readonly path: string };
...
function permissionFor(authClient: AdminRbacAuthorizationSource | undefined, action: string, fallback: boolean): boolean {
  const candidate = authClient as
    | (AdminRbacAuthorizationSource & {
...
function Shell({ locale, path, screenId, state, title, description, actions, children }: { readonly locale: SupportedLocale; readonly path: string; readonly screenId: string; readonly state: AdminRbacState; readonly title?: ReactNode; readonly description?: ReactNode; readonly actions?: ReactNode; readonly children: ReactNode }) {
  return (
    <section className="admin-rbac" data-screen-id={screenId} data-route={path} data-device-scope="desktop" data-state={state} dir={locale === "ar" ? "rtl" : "ltr"}>
      <AdminNavigation locale={locale} activePath={path} />
      <div className="admin-rbac__content">
        {title === undefined ? null : (
          <header className="admin-rbac__heading">
            <div>
              <h1>{title}</h1>
...
function RbacLink({ href, children, primary = false }: { readonly href: string; readonly children: ReactNode; readonly primary?: boolean }) {
  return (
    <a className={primary ? "ui-button ui-button--primary ui-button--md" : "admin-rbac__secondary-link"} href={href}>
      {children}
    </a>
  );
}
...
function Field({ label, id, value, onChange, type = "text", required = false, disabled = false }: { readonly label: string; readonly id: string; readonly value: string; readonly onChange: (value: string) => void; readonly type?: "text" | "email"; readonly required?: boolean; readonly disabled?: boolean }) {
  return (
    <label className="admin-rbac__field" htmlFor={id}>
      {label}
      <input id={id} type={type} value={value} required={required} disabled={disabled} onChange={(event) => onChange(event.target.value)} />
    </label>
  );
}
...
function ReasonField({ copy, value, onChange, disabled }: { readonly copy: ReturnType<typeof getAdminRbacCopy>; readonly value: string; readonly onChange: (value: string) => void; readonly disabled?: boolean }) {
  return (
    <label className="admin-rbac__field" htmlFor="admin-rbac-reason">
      {copy.reason}
      <textarea id="admin-rbac-reason" required minLength={3} maxLength={1_000} value={value} disabled={disabled} placeholder={copy.reasonPlaceholder} onChange={(event) => onChange(event.target.value)} />
    </label>
  );
}
...
function UsersList({ locale, source, authClient }: { readonly locale: SupportedLocale; readonly source: AdminRbacSource; readonly authClient?: AdminRbacAuthorizationSource | undefined }) {
  ...
  const metrics = [
    [copy.users, data?.total ?? "—"],
    [copy.activeStatus, visible.filter((user) => user.status === "active").length],
    [copy.disabledStatus, visible.filter((user) => user.status === "disabled").length],
    [copy.superAdmin, visible.filter((user) => user.accessLevel === "super_admin").length],
    [copy.standardAdmin, visible.filter((user) => user.accessLevel === "standard_admin").length],
    [locale === "ar" ? "في الصفحة الحالية" : "On this page", visible.length],
  ] as const;
  return (
    <Shell
      locale={locale}
      path={ADMIN_RBAC_USERS_ROUTE}
      screenId="ADM-59"
      state={state}
      title={copy.users}
      description={copy.usersDescription}
      actions={
        <div className="admin-rbac__heading-actions">
          <RbacLink href={localePath(locale, ADMIN_RBAC_ROLES_ROUTE)}>{copy.roles}</RbacLink>
          {canCreate ? (
            <RbacLink primary href={localePath(locale, `${ADMIN_RBAC_USERS_ROUTE}/new`)}>
              {copy.addUser}
            </RbacLink>
          ) : null}
        </div>
    ...
    >
      ...
      <div className="admin-rbac__status-tabs" aria-label={copy.filterStatus}>
        {(["", "active", "disabled"] as const).map((status) => (
          <button
            key={status || "all"}
            type="button"
            data-active={(query.status ?? "") === status}
            onClick={() =>
              setQuery((current) => ({
                ...current,
                page: 1,
          ...
          >
            {status === "" ? copy.all : status === "active" ? copy.activeStatus : copy.disabledStatus}
          </button>
        ))}
      </div>
      <div className="admin-rbac__filters">
        <label htmlFor="admin-rbac-access">
          {copy.filterAccess}
          <select
            id="admin-rbac-access"
            value={query.accessLevel ?? ""}
            onChange={(event) =>
              setQuery((current) => ({
                ...current,
                page: 1,
                accessLevel: event.target.value === "" ? undefined : (event.target.value as AdminUserListQuery["accessLevel"]),
              }))
            }
          >
            <option value="">{copy.all}</option>
            <option value="super_admin">{copy.superAdmin}</option>
            <option value="standard_admin">{copy.standardAdmin}</option>
          </select>
        </label>
      </div>
      ...
      {state === "success" && data !== undefined ? (
        <section className="admin-rbac__panel">
          <table className="admin-rbac__table">
            ...
            <thead>
              <tr>
                <th scope="col">{copy.displayName}</th>
            ...
            <tbody>
              {data.items.map((user) => (
                <tr key={user.id}>
                  ...
                  <td>{user.status === "active" ? copy.activeStatus : copy.disabledStatus}</td>
                  <td>{user.version}</td>
                  <td>
                    {user.availableActions.includes("update") ? (
                      <a className="admin-rbac__row-link" href={localePath(locale, `${ADMIN_RBAC_USERS_ROUTE}/${user.id}`)}>
                        {copy.edit}
                      </a>
                    ) : (
          ...
          <div className="admin-rbac__pagination">
            <Button size="sm" variant="secondary" disabled={query.page <= 1} onClick={() => setQuery((current) => ({ ...current, page: current.page - 1 }))}>
              {copy.back}
            </Button>
            <span aria-live="polite">
              {query.page} / {pageCount}
            </span>
            <Button size="sm" variant="secondary" disabled={query.page >= pageCount} onClick={() => setQuery((current) => ({ ...current, page: current.page + 1 }))}>
              {copy.retry}
            </Button>
          </div>
        </section>
...
function UserCreate({ locale, source, authClient }: { readonly locale: SupportedLocale; readonly source: AdminRbacSource; readonly authClient?: AdminRbacAuthorizationSource | undefined }) {
  ...
  return (
    <Shell locale={locale} path={`${ADMIN_RBAC_USERS_ROUTE}/new`} screenId="ADM-60" state={state} title={copy.addUser} actions={<RbacLink href={localePath(locale, ADMIN_RBAC_USERS_ROUTE)}>{copy.back}</RbacLink>}>
      <section className="admin-rbac__form-panel">
        <div className="admin-rbac__panel-heading">
          <div>
            <h2>{locale === "ar" ? "البيانات الأساسية" : "Basic information"}</h2>
          </div>
        </div>
        <form
          onSubmit={(event) => {
        ...
        >
          <div className="admin-rbac__form-grid">
            <Field label={copy.displayName} id="admin-rbac-display-name" value={form.displayName} required disabled={!canCreate || saving} onChange={(displayName) => setForm((current) => ({ ...current, displayName }))} />
            <Field label={copy.email} id="admin-rbac-email" type="email" value={form.email} required disabled={!canCreate || saving} onChange={(email) => setForm((current) => ({ ...current, email }))} />
            <label className="admin-rbac__field" htmlFor="admin-rbac-access-level">
              {copy.accessLevel}
              <select
                id="admin-rbac-access-level"
                value={form.accessLevel}
                disabled={!canCreate || saving}
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    accessLevel: event.target.value as AdminUserCreate["accessLevel"],
                  }))
                }
              >
                <option value="standard_admin">{copy.standardAdmin}</option>
          ...
          {feedback !== undefined ? (
            <p className="admin-rbac__feedback" role="alert">
              {feedback}
            </p>
          ) : null}
        </form>
...
function UserDetail({ id, locale, source }: { readonly id: string; readonly locale: SupportedLocale; readonly source: AdminRbacSource }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>, status?: AdminUserPatch["status"]): Promise<void> {
    ...
    setSaving(true);
    try {
      const next = await source.updateUser(id, parsed.data);
      setUser(next);
      setForm({
        expectedVersion: next.version,
        reason: "",
      ...
      setState(stateForError(error, true));
      setFeedback(validationMessage(error, copy.validation));
    } finally {
      setSaving(false);
    }
  ...
  if (state === "loading")
    return (
      <Shell locale={locale} path={`${ADMIN_RBAC_USERS_ROUTE}/${id}`} screenId="ADM-61" state={state}>
        <StatePanel state="loading" locale={locale} onRetry={() => setAttempt((value) => value + 1)} />
      </Shell>
    );
  if (state === "not_found" || state === "permission" || state === "error" || state === "retry" || user === undefined)
    return (
      <Shell locale={locale} path={`${ADMIN_RBAC_USERS_ROUTE}/${id}`} screenId="ADM-61" state={state}>
        <StatePanel state={panelState(state)} locale={locale} onRetry={() => setAttempt((value) => value + 1)} />
      </Shell>
    );
  ...
  return (
    <Shell locale={locale} path={`${ADMIN_RBAC_USERS_ROUTE}/${id}`} screenId={screenId} state={state} title={user.displayName} description={`${user.email} · ${user.accessLevel === "super_admin" ? copy.superAdmin : copy.standardAdmin}`} actions={<span className="admin-rbac__status">{user.status === "active" ? copy.activeStatus : copy.disabledStatus}</span>}>
      <section className="admin-rbac__form-panel">
        <div className="admin-rbac__panel-heading">
          <div>
        ...
        <dl className="admin-rbac__details">
          <div>
            <dt>{copy.version}</dt>
            <dd>{user.version}</dd>
          </div>
        ...
        {canUpdate ? (
          ...
          >
            <div className="admin-rbac__form-grid">
              <Field label={copy.displayName} id="admin-rbac-detail-name" value={form.displayName ?? ""} disabled={saving} onChange={(displayName) => setForm((current) => ({ ...current, displayName }))} />
              <Field label={copy.email} id="admin-rbac-detail-email" type="email" value={form.email ?? ""} disabled={saving} onChange={(email) => setForm((current) => ({ ...current, email }))} />
              <label className="admin-rbac__field" htmlFor="admin-rbac-detail-level">
                {copy.accessLevel}
                <select
                  id="admin-rbac-detail-level"
                  value={form.accessLevel ?? user.accessLevel}
                  disabled={saving}
                  onChange={(event) =>
                    setForm((current) => ({
                      ...current,
                      accessLevel: event.target.value as AdminUserPatch["accessLevel"],
                    }))
                  }
                >
                  <option value="standard_admin">{copy.standardAdmin}</option>
            ...
            <div className="admin-rbac__actions">
              ...
              {canDisable ? (
                <Button
                  ...
                  disabled={saving}
                  onClick={(event) => {
                    void submit(event as unknown as FormEvent<HTMLFormElement>, "disabled");
                  }}
                >
                  {copy.disable}
                </Button>
              ...
              {canEnable ? (
                <Button
                  ...
                  disabled={saving}
                  onClick={(event) => {
                    void submit(event as unknown as FormEvent<HTMLFormElement>, "active");
                  }}
                >
                  {copy.enable}
                </Button>
...
function RoleList({ locale, source }: { readonly locale: SupportedLocale; readonly source: AdminRbacSource }) {
  ...
  const [form, setForm] = useState({
    name: "",
    description: "",
    accessMode: "custom" as "custom" | "view_only",
  ...
  async function create(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault();
    setFeedback(undefined);
    const parsed = rbacRoleCreateRequestSchema.safeParse(form);
    if (!parsed.success) {
      setFeedback(copy.validation);
      return;
    }
    setCreating(true);
    try {
      await source.createRole(parsed.data);
      setFeedback(copy.saved);
  ...
  return (
    <Shell locale={locale} path={ADMIN_RBAC_ROLES_ROUTE} screenId="ADM-63" state={state} title={copy.roles} description={copy.rolesDescription} actions={<RbacLink href={localePath(locale, ADMIN_RBAC_USERS_ROUTE)}>{copy.users}</RbacLink>}>
      ...
      {state === "empty" ? (
        <section className="admin-rbac__state" data-state="empty" aria-label={copy.states.empty.title}>
          <StateMessage state="empty" title={copy.states.empty.title} message={copy.noRoles} />
        </section>
      ) : null}
      {state === "success" && data !== undefined ? (
        <>
          <section className="admin-rbac__panel">
            <table className="admin-rbac__table">
              <caption className="a11y-visually-hidden">{copy.roles}</caption>
              <thead>
                <tr>
                  <th scope="col">{copy.roleName}</th>
                  <th scope="col">{copy.accessMode}</th>
                  <th scope="col">{copy.permissions}</th>
              ...
              <tbody>
                {data.items.map((role) => (
                  <tr key={role.id}>
                    <td>
                      <a className="admin-rbac__row-link" href={localePath(locale, `${ADMIN_RBAC_ROLES_ROUTE}/${role.id}`)}>
                        {role.name}
                      </a>
                      <small>
                        <code>{role.id}</code>
                    ...
                    <td>{role.accessMode === "view_only" ? copy.viewOnly : copy.custom}</td>
                    <td>{role.permissions.length}</td>
                    <td>{role.active ? copy.activeStatus : copy.disabledStatus}</td>
                    <td>
                      {role.availableActions.includes("update") ? (
                        <a className="admin-rbac__row-link" href={localePath(locale, `${ADMIN_RBAC_ROLES_ROUTE}/${role.id}`)}>
                          {copy.edit}
                        </a>
                      ) : (
          ...
          {canManage ? (
            <details className="admin-rbac__form-panel">
              <summary>{copy.createRole}</summary>
              <form
                onSubmit={(event) => {
                  void create(event);
                }}
              >
                <div className="admin-rbac__form-grid">
                  <Field label={copy.roleName} id="admin-rbac-role-name" value={form.name} disabled={creating} onChange={(name) => setForm((current) => ({ ...current, name }))} />
                  <Field label={copy.description} id="admin-rbac-role-description" value={form.description} disabled={creating} onChange={(description) => setForm((current) => ({ ...current, description }))} />
                  <label className="admin-rbac__field" htmlFor="admin-rbac-role-mode">
                    {copy.accessMode}
                    <select
                      id="admin-rbac-role-mode"
                      value={form.accessMode}
                      disabled={creating}
                      onChange={(event) =>
                        setForm((current) => ({
                          ...current,
                          accessMode: event.target.value as "custom" | "view_only",
                          permissions: event.target.value === "view_only" ? current.permissions.filter((item) => item.endsWith(".view")) : current.permissions,
                        }))
                      }
                    >
                      <option value="custom">{copy.custom}</option>
                      <option value="view_only">{copy.viewOnly}</option>
                    </select>
                  </label>
                </div>
...
function RoleDetail({ id, locale, source }: { readonly id: string; readonly locale: SupportedLocale; readonly source: AdminRbacSource }) {
  ...
  useEffect(() => {
    ...
    void source
      ...
      .then((next) => {
        if (controller.signal.aborted) return;
        const found = next.items.find((item) => item.id === id);
        setData(next);
        setRole(found);
        if (found !== undefined)
          setForm({
        ...
        setState(found === undefined ? "not_found" : "success");
      })
      .catch((error) => {
        if (!controller.signal.aborted) setState(stateForError(error, true));
      });
  ...
  if (state === "loading")
    return (
      <Shell locale={locale} path={`${ADMIN_RBAC_ROLES_ROUTE}/${id}`} screenId="ADM-64" state={state}>
        <StatePanel state="loading" locale={locale} onRetry={() => setAttempt((value) => value + 1)} />
      </Shell>
    );
  if (role === undefined || data === undefined)
    return (
      <Shell locale={locale} path={`${ADMIN_RBAC_ROLES_ROUTE}/${id}`} screenId="ADM-64" state={state}>
        <StatePanel state={panelState(state)} locale={locale} onRetry={() => setAttempt((value) => value + 1)} />
      </Shell>
  ...
  async function save(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault();
    setFeedback(undefined);
    const parsed = rbacRolePatchRequestSchema.safeParse({
    ...
    setSaving(true);
    try {
      const next = await source.updateRole(id, parsed.data);
      setRole(next);
      setForm((current) => ({
        ...current,
    ...
    <Shell locale={locale} path={`${ADMIN_RBAC_ROLES_ROUTE}/${id}`} screenId="ADM-64" state={state} title={role.name} description={copy.rolesDescription} actions={<RbacLink href={localePath(locale, ADMIN_RBAC_ROLES_ROUTE)}>{copy.back}</RbacLink>}>
      <section className="admin-rbac__form-panel">
        {canUpdate ? (
          ...
          >
            <div className="admin-rbac__form-grid">
              <Field label={copy.roleName} id="admin-rbac-role-detail-name" value={form.name} disabled={saving} onChange={(name) => setForm((current) => ({ ...current, name }))} />
              <Field label={copy.description} id="admin-rbac-role-detail-description" value={form.description} disabled={saving} onChange={(description) => setForm((current) => ({ ...current, description }))} />
              <label className="admin-rbac__field" htmlFor="admin-rbac-role-detail-mode">
                {copy.accessMode}
                <select
                  id="admin-rbac-role-detail-mode"
                  value={form.accessMode}
                  disabled={saving}
                  onChange={(event) =>
                    setForm((current) => ({
            ...
            <label className="admin-rbac__checkbox">
              <input
                type="checkbox"
                checked={form.active}
                disabled={saving}
...
export function AdminRbac({ url, locale, session, authClient, apiOrigin, source: providedSource }: AdminRbacProps) {
  ...
  if (session.status !== "authenticated" || session.role !== "admin")
    return (
      <Shell locale={locale} path={path} screenId="ADM-59" state="permission">
        <StatePanel state="permission" locale={locale} onRetry={() => undefined} />
      </Shell>
    );
  if (view.kind === "not-found")
    return (
      <Shell locale={locale} path={path} screenId="ADM-59" state="not_found">
