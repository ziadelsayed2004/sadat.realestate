...
function PropertyMetricStrip({ data, locale, review }: { readonly data: AdminPropertyListData; readonly locale: SupportedLocale; readonly review?: boolean }) {
  ...
  const labels = review
    ? [copy.metrics.total, copy.status.pending_review, copy.status.needs_changes, copy.status.approved, copy.status.rejected, copy.status.hidden, copy.status.archived, copy.metrics.loaded]
    : [copy.metrics.total, copy.status.published, copy.status.pending_review, copy.status.needs_changes, copy.status.draft, copy.status.hidden, copy.status.archived];
...
function StatePanel({ state, locale, onRetry }: { readonly state: Exclude<AdminPropertiesState, 'success' | 'empty' | 'not_found'>; readonly locale: SupportedLocale; readonly onRetry: () => void }) {
  ...
  return (
    <section className="admin-properties__state" data-state={state} aria-label={message.title}>
      <StateMessage state={state} loadingVariant="table" title={message.title} message={message.body} onRetry={state === 'retry' ? onRetry : undefined} retryLabel={copy.retry} />
...
export function AdminProperties({ locale, session, authClient, apiOrigin, url, view, propertyId, reportId, initialProperties, initialDuplicates, initialReports, initialState = 'loading', loadProperties, loadDuplicates, loadReports, reviewProperty, changeVisibility, resolveReport }: AdminPropertiesProps) {
  ...
  useEffect(() => {
    ...
    if (initialProperties !== undefined && view !== 'reports' && view !== 'duplicates' && attempt === 0) { const selected = selectedPropertyId === undefined ? undefined : initialProperties.items.find(item => item.id === selectedPropertyId); setState(view === 'review' && selectedPropertyId !== undefined && selected === undefined ? 'not_found' : stateForData(initialProperties.items)); return undefined; }
  ...
  const showPropertyList = view === 'list' || ((view === 'review' || view === 'duplicates') && selectedPropertyId === undefined);
  ...
  return <section className="admin-properties" data-screen-id={screenId} data-route={pathname} data-device-scope="desktop" data-admin-properties-state={state}><AdminNavigation locale={locale} activePath={pathname} /><div className="admin-properties__content"><PageHeading copy={copy} view={view} />{showPropertyList ? <>{state === 'success' && properties !== undefined ? <PropertyMetricStrip data={properties} locale={locale} /> : null}<PropertyStatusStrip locale={locale} selected={(propertyQuery.status as PropertyStatus | undefined) ?? ''} onSelect={status => { setPropertyQuery(current => ({ ...current, page: 1, ...(status === '' ? { status: undefined } : { status }) })); setAttempt(value => value + 1); }} /><PropertyFilters locale={locale} activeStatus={(propertyQuery.status as PropertyStatus | undefined)} onApply={query => { setPropertyQuery({ page: 1, limit: 20, sort: 'updatedAt', direction: 'desc', ...query }); setAttempt(value => value + 1); }} onClear={() => { setPropertyQuery({ page: 1, limit: 20, sort: 'updatedAt', direction: 'desc' }); setAttempt(value => value + 1); }} /></> : null}{view === 'reports' ? <ReportFilters locale={locale} onApply={query => { setReportQuery({ page: 1, limit: 20, ...query }); setAttempt(value => value + 1); }} onClear={() => { setReportQuery({ page: 1, limit: 20 }); setAttempt(value => value + 1); }} /> : null}{state === 'loading' || state === 'retry' || state === 'error' || state === 'permission' ? <StatePanel state={state} locale={locale} onRetry={retry} /> : null}{state === 'not_found' ? <NotFoundPanel locale={locale} /> : null}{showPropertyList && state === 'empty' ? <section className="admin-properties__empty" data-state="empty"><h2>{copy.states.empty.title}</h2><p>{copy.states.empty.body}</p></section> : null}{showPropertyList && state === 'success' && properties !== undefined ? <PropertyTable locale={locale} data={properties} onPage={page => { setPropertyQuery(current => ({ ...current, page })); setAttempt(value => value + 1); }} /> : null}{view === 'review' && !showPropertyList && state === 'success' && properties !== undefined ? <PropertyMetricStrip data={properties} locale={locale} review /> : null}{view === 'review' && state === 'success' && selectedProperty !== undefined ? <PropertyReviewPanel locale={locale} property={selectedProperty} review={reviewMutation} visibility={visibilityMutation} /> : null}{view === 'duplicates' && state === 'success' && selectedPropertyId !== undefined && duplicates !== undefined ? <DuplicatePanel locale={locale} propertyId={selectedPropertyId} data={duplicates} /> : null}{view === 'reports' && state === 'empty' ? <section className="admin-properties__empty" data-state="empty"><h2>{copy.noReports}</h2><p>{copy.states.empty.body}</p></section> : null}{view === 'reports' && state === 'success' && reports !== undefined ? <><ReportsTable locale={locale} data={reports} />{selectedReport !== undefined ? <section className="admin-properties__report-detail"><div><p className="admin-properties__eyebrow">{copy.details}</p><h2><code>{selectedReport.id}</code></h2><dl><div><dt>{copy.columns.propertyId}</dt><dd><code>{selectedReport.propertyId}</code></dd></div><div><dt>{copy.columns.reportReason}</dt><dd>{copy.reportReason[selectedReport.reason]}</dd></div><div><dt>{copy.columns.status}</dt><dd>{copy.reportStatus[selectedReport.status]}</dd></div>{selectedReport.details !== undefined ? <div><dt>{copy.detailsLabel}</dt><dd>{selectedReport.details}</dd></div> : null}</dl></div><ReportResolutionPanel locale={locale} report={selectedReport} resolve={reportResolver} /></section> : null}</> : null}</div></section>;
}
