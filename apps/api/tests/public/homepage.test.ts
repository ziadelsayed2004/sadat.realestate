...
function sources(overrides: Partial<HomepageSources> = {}): HomepageSources {
  return {
    ...
    banners: [
      { key: 'hero', title: localized, order: 0, status: 'published', active: true, imageUrl: 'https://cdn.example/hero.webp' },
...
test('drops malformed persisted public rows and supports a safe empty state', async () => {
  ...
  assert.deepEqual(await service.read(), { sections: [], properties: [], developers: [], content: [], banners: [] });
});
