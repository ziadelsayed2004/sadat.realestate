...
test('exposes customer names only through owned appointment access', async () => {
  ...
  const service = createViewingService({ authorization: { authorize: async () => true }, repository: createInMemoryViewingRepository([seed]) });
  assert.equal((await service.list(provider, {})).items[0]?.customerName, 'Local Customer');
  const stranger = { ...provider, sub: '5123456789abcdef01234567' };
  assert.equal((await service.list(stranger, {})).total, 0);
  await assert.rejects(() => service.get(stranger, viewingId), /VIEWING_NOT_FOUND/);
  await assert.rejects(() => service.transition(stranger, viewingId, { action: 'confirm', expectedVersion: 0 }), /VIEWING_NOT_FOUND/);
});
