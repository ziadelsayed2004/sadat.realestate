...
export function createAdSettingsService(seed: { placements?: AdPlacement[]; settings?: AdSettings; banners?: AdBanner[]; bannerMedia?: AdBannerMedia[]; now?: () => Date } = {}) {
  ...
  return {
    ...
    async createRequest(claims: AccessTokenClaims, input: unknown) {
      ...
      if (![...placements.values()].some(item => item.key === parsed.placementKey && item.active)) throw new AdSettingsServiceError('NOT_FOUND');
      const stamp = now();
    ...
    async decideQuote(claims: AccessTokenClaims, quoteId: string, input: unknown) {
      ...
      if (parsed.action === 'accept') {
        ...
        if (quote.status !== 'issued' || new Date(quote.validUntil) <= new Date()) throw new AdSettingsServiceError('VERSION_CONFLICT');
      } else {
        requireAdmin(claims);
