...
export interface PublicPropertyDetailsSource {
  ...
  relatedProperties: Array<{ id: string; slug: string; kind: string; name: unknown; transactionType: string; imageUrl?: string; projectId?: string; description?: unknown; area?: unknown; layout?: unknown; price?: unknown; status: string; active: boolean }>;
}

export interface PublicPropertyDetailsRepository { findBySlug(slug: string): Promise<PublicPropertyDetailsSource | null> }

function card(source: { id: string; slug: string; kind: string; name: unknown; transactionType: string; imageUrl?: string; projectId?: string; description?: unknown; area?: unknown; layout?: unknown; price?: unknown; status: string; active: boolean }) {
  const parsed = publicHomepagePropertySchema.safeParse({ id: source.id, slug: source.slug, kind: source.kind, name: source.name, transactionType: source.transactionType, ...(source.imageUrl ? { imageUrl: source.imageUrl } : {}), ...(source.projectId ? { projectId: source.projectId } : {}), ...(source.description !== undefined ? { description: source.description } : {}), ...(source.area !== undefined ? { area: source.area } : {}), ...(source.layout !== undefined ? { layout: source.layout } : {}), ...(source.price !== undefined ? { price: source.price } : {}) });
...
function property(row: Row): PublicPropertyDetailsSource | null {
  ...
  return { id: rowId, slug, kind, name, transactionType, sourceType, ...(typeof row.imageUrl === 'string' ? { imageUrl: row.imageUrl } : {}), ...(organizationId ? { organizationId } : {}), ...(projectId ? { projectId } : {}), ...(row.description !== undefined ? { description: row.description } : {}), ...(row.area !== undefined ? { area: row.area } : {}), ...(row.layout !== undefined ? { layout: row.layout } : {}), ...(row.price !== undefined ? { price: row.price } : {}), status, active, media: [], relatedProperties: [] };
}
...
export function createMongoosePublicPropertyDetailsRepository(connection: Connection): PublicPropertyDetailsRepository {
  return { async findBySlug(slug) {
    ...
    const row = await properties.findOne({ slug, status: 'published', active: true }, { projection: { _id: 1, slug: 1, kind: 1, name: 1, transactionType: 1, imageUrl: 1, sourceType: 1, organizationId: 1, projectId: 1, description: 1, area: 1, layout: 1, price: 1, status: 1, active: 1 } });
    ...
    const relatedProperties = relatedRows.flatMap((value) => { const mapped = property(value as Row); return mapped ? [{ id: mapped.id, slug: mapped.slug, kind: mapped.kind, name: mapped.name, transactionType: mapped.transactionType, ...(mapped.imageUrl ? { imageUrl: mapped.imageUrl } : {}), ...(mapped.projectId ? { projectId: mapped.projectId } : {}), ...(mapped.description !== undefined ? { description: mapped.description } : {}), ...(mapped.area !== undefined ? { area: mapped.area } : {}), ...(mapped.layout !== undefined ? { layout: mapped.layout } : {}), ...(mapped.price !== undefined ? { price: mapped.price } : {}), status: mapped.status, active: mapped.active }] : []; });
