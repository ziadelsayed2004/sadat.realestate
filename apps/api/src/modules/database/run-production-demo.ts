...
if (isEntrypoint()) {
  ...
  const action = operation === 'reset' ? resetProductionDemo : undefined;
  if (!action) {
    process.stderr.write('Usage: run-production-demo.js reset\n');
    process.exitCode = 2;
  } else {
    action().then(count => {
      process.stdout.write(`PRODUCTION_DEMO_RESET count=${count}\n`);
    }).catch(() => {
