...
export function createCommissionPolicyService(seed: { policies?: CommissionPolicy[]; now?: () => Date; repository?: CommissionPolicyRepository } = {}) {
  ...
  const all = async (): Promise<CommissionPolicy[]> => repository ? repository.list() : [...policies.values()];
  const get = async (policyId: string) => {
    const policy = repository ? await repository.findById(policyId) : policies.get(policyId);
    if (!policy) throw new CommissionPolicyServiceError('COMMISSION_NOT_FOUND');
    return policy;
  };
  const validateActive = (policy: CommissionPolicy, existing: readonly CommissionPolicy[]) => {
  ...
  return {
    ...
    async updatePolicy(claims: AccessTokenClaims, policyId: string, input: unknown) {
      admin(claims);
      const current = await get(policyId);
      ...
      validateActive(next, await all());
      if (repository) {
        const result = await repository.replace(next, current.version);
        if (result.kind === 'not_found') throw new CommissionPolicyServiceError('COMMISSION_NOT_FOUND');
        if (result.kind === 'version_conflict') throw new CommissionPolicyServiceError('COMMISSION_VERSION_CONFLICT');
      } else {
...
export type CommissionPolicyService = ReturnType<typeof createCommissionPolicyService>;
