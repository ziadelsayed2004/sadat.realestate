...
export interface PublicPropertyComparisonSource { id: string; slug: string; kind: string; name: unknown; transactionType: string; imageUrl?: string; projectId?: string; description?: unknown; area?: unknown; layout?: unknown; price?: unknown; status: string; active: boolean }
...
export function createMongoosePublicPropertyComparisonRepository(connection: Connection): PublicPropertyComparisonRepository {
  return { async findPublished(ids) {
    const rows = await connection.collection('properties').find({ _id: { $in: ids.map((value) => new Types.ObjectId(value)) }, status: 'published', active: true }, { projection: { _id: 1, slug: 1, kind: 1, name: 1, transactionType: 1, imageUrl: 1, projectId: 1, description: 1, area: 1, layout: 1, price: 1, status: 1, active: 1 } }).toArray();
