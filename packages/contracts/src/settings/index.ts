...
export const publicSeoSettingsSchema=z.object({title:localizedTextSchema,description:localizedTextSchema,canonicalUrl:url,robots:seoSettingsSchema.shape.robots,titleSeparator:z.string().trim().min(1).max(10).optional(),sitemapStatus:z.enum(['active','inactive']).default('active'),googleSiteVerification:z.string().trim().min(1).max(512).optional()}).strict();export const publicPrivacyPolicySchema=z.object({key:safeKey,title:localizedTextSchema,body:localizedTextSchema,effectiveAt:z.string().datetime({offset:true}).nullable()}).strict();
export const publicSeoSettingsSuccessEnvelopeSchema=successEnvelopeSchema(publicSeoSettingsSchema);
export type SeoSettings=z.infer<typeof seoSettingsSchema>;export type SeoSettingsPatch=z.infer<typeof seoSettingsPatchSchema>;export type PublicSeoSettings=z.infer<typeof publicSeoSettingsSchema>;export type PrivacyPolicy=z.infer<typeof privacyPolicySchema>;export type PrivacyPolicyPatch=z.infer<typeof privacyPolicyPatchSchema>;
