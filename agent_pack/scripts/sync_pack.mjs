...
for (const task of catalog) {
  ...
  const programSuffix = activeIds.has(task.id) ? ' — ACTIVE_PROGRAM' : '';
  lines.push('- [' + box + '] ' + task.id + ' — ' + task.title + suffix + programSuffix);
}
...
const activeByStatus = Object.fromEntries(statuses.map((s) => [s, activeProgram.activeTaskIds.filter((id) => state.tasks[id]?.status === s).length]));
const historicalByStatus = Object.fromEntries(statuses.map((s) => [s, catalog.filter((task) => !activeIds.has(task.id) && state.tasks[task.id]?.status === s).length]));
write('03_execution/COUNT_SUMMARY.json', { total: catalog.length, byTrack, byStatus, activeProgram: { programId: activeProgram.programId, total: activeProgram.activeTaskIds.length, byStatus: activeByStatus }, historical: { total: catalog.length - activeProgram.activeTaskIds.length, byStatus: historicalByStatus }, selectedTaskId: null, updatedAt: new Date().toISOString() });
