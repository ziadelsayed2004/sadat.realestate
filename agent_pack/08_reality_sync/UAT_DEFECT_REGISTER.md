...
| READINESS-DOCKER-001 | External prerequisite | Container smoke | Deferred | Docker is not installed in the current environment. Checked-in container safety is covered by API artifact tests; engine-backed Compose/image health evidence requires an approved local or CI Docker runner. |
