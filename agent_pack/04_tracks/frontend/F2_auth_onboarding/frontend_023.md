...
| Kind | frontend |
| Sequence | 139 / 199 |
| Depends on | `frontend_022` |

## Goal

Implement AUTH-09 variants, validation, save, and resume behavior.
...
- Screen ID `AUTH-09` through `09_sources/DESIGN_SOURCE_MANIFEST.json`.
- Screen ID `AUTH-09+` through `09_sources/DESIGN_SOURCE_MANIFEST.json`.
...
Create `07_finish/frontend_023/completion.json`, then use the status tool and run sync, audit, and selector. Stop after this task.
