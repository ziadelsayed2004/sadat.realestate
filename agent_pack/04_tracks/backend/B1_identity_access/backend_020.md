...
Implement an append-only AuditLog that records actor, target, action, reason, before, after, and trace data while redacting secrets.
...
Create `07_finish/backend_020/completion.json`, then use the status tool and run sync, audit, and selector. Stop after this task.
