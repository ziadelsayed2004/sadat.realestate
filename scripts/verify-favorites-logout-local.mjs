...
try {
  ...
  for (const locale of ['ar', 'en']) {
    for (const [device, preset] of [['desktop', 'Desktop Chrome'], ['tablet', 'Galaxy Tab S4'], ['mobile', 'Pixel 5']]) {
      ...
      try {
        ...
        const read = await context.request.get(`${base}/api/v1/seeker/favorites`, { headers });
        assert.equal(read.status(), 200);
        const loggedOut = await context.request.post(`${base}/api/v1/auth/logout`, { data: {} });
        assert.equal(loggedOut.status(), 200);
        const propertyId = new Types.ObjectId().toHexString();
        const results = [];
        for (const method of ['GET', 'PUT', 'DELETE']) {
          const response = await context.request.fetch(`${base}/api/v1/seeker/favorites${method === 'GET' ? '' : `/${propertyId}`}`, { method, headers });
          assert.equal(response.status(), 401);
          results.push({ method, httpStatus: response.status() });
        }
  ...
  try {
    ...
    await writeFile('docs/quality/guide-runs/favorites-logout-local-latest.json', `${JSON.stringify(report, null, 2)}\n`);
  }
}
