...
const adminEvidence = `<section><h2>تحديث لوحة الإدارة وProduction — 8 سبتمبر 2026</h2>
<p class="ok"><strong>تحقق هذه الجولة محليًا:</strong> 54/54 حالة متصفح للنظرة العامة والسايدبار والمقالات والتصنيفات والكوميونيتي والتعليقات والبلاغات عبر Desktop/Tablet/Mobile بالعربية والإنجليزية؛ و31/31 اختبار واجهة و19/19 اختبار خدمات ومسارات API للمحتوى. التصنيفات الثمانية في السايدبار تقبل الفتح والقفل مع حفظ الاختيار، وضُبطت مسافات كروت النظرة العامة والكوميونيتي من مرجع Figma.</p>
<p class="warn"><strong>فجوة مؤكدة:</strong> نشر/إخفاء منشورات الكوميونيتي من الإدارة غير مربوط بواجهة ومسار API؛ معالجة البلاغات والمقالات اختُبرت، لكن ذلك لا يثبت اكتمال اعتماد منشورات المجتمع. رقم 314/314 السابق غير معتمد لعدم توفر نتيجة اكتماله للتحقق. الأرقام الحالية ليست نسبة مطابقة Figma ولا اختبارًا جديدًا لكل شاشات الإدارة.</p>
<p class="warn"><strong>Production:</strong> تسجيل الدخول للقراءة فقط نجح، لكن الحساب المستخدم ظهر كـ SEK-01 seeker. مسارات الإدارة الستة أعادت forbidden على Desktop بعرض 1440 وعلى Pixel 5 بعرض 393، مع صفر overflow وصفر أخطاء HTTP/request/console؛ لذلك صلاحية Admin الحية لم تثبت بهذا الحساب.</p>
<p>تم جلب سياق Figma مباشرةً لـ ADM-01 (<code>6017:61879</code>) والـSidebar (<code>6017:62727</code>)، واستبدال أيقونات Unicode بأصول SVG محلية canonical. ثُبتت أبعاد عناصر select في شاشات الإدارة وCMS، وأعيدت لقطات الحالات المتأثرة؛ اختبار CMS المرئي الحالي مستقر <strong>6/6</strong>. ما زالت المطابقة البصرية المباشرة الكاملة غير مقاسة: 22/131 Desktop و0/131 Responsive، لذلك لا يصح إعلان 100%.</p></section>`;
...
<p>8 سبتمبر 2026 · ${source.rows.length} شاشة/حالة. المرجع البصري المعتمد هو ملف Figma <code>Odl1Epn2u6lIEuIMmABT7o</code>، ودليل المستخدم مرجع للآلية مع اعتماد البريد الإلكتروني وOTP بدل التسجيل القديم بالهاتف.</p>
<section><h2>الخلاصة بالأرقام</h2><div class="cards">
<div class="card"><strong>${source.rows.length}</strong>إجمالي الشاشات/الحالات المسجلة</div><div class="card"><strong>${measuredDesktop}/${source.rows.length}</strong>نسب Figma الحالية المقاسة للديسكتوب</div><div class="card"><strong>${measuredResponsive}/${source.rows.length}</strong>نسب Figma الحالية المقاسة للريسبونسيف</div><div class="card"><strong>72/72</strong>فحص صفحات Public كاملًا + 72/72 للفوتر</div></div>
<p class="warn"><strong>لا توجد حتى الآن نسبة مطابقة حديثة موثوقة للصفحات الكاملة؛ «غير مقاسة» لا تعني 0% ولا 100%.</strong> القياسات التاريخية تعرض فرقًا قديمًا فقط، ولا يصح تحويلها إلى نسبة حالية بطرحها من 100.</p><p>${groups.map(([name, values]) => `${escape(name)}: ${values.length}`).join(' · ')}</p></section>${adminEvidence}
...
<p>وفحص الصفحة الكاملة اجتاز أيضًا 72/72 حالة: HTTP 200، بلا خروج أفقي للـdocument أو صور مكسورة أو أخطاء متصفح. أُصلح كذلك خروج شارات كروت العقارات إلى أعلى الصفحة، وأصبح التدقيق يفشل تلقائيًا إذا خرجت أي شارة من حدود كارتها. العناصر الداخلية المتحركة والجداول القابلة للسحب تظل محفوظة كتفاصيل تشخيصية.</p><p><a href="public-responsive-2026-09-06/results.json">نتائج الصفحات الكاملة</a> · <a href="public-responsive-2026-09-06/pub-01-badges-fixed.png">لقطة إصلاح الشارات</a> · <a href="public-footer-responsive-2026-09-06/results.json">نتائج الفوتر</a> · <a href="public-footer-responsive-2026-09-06/PUB-01-ar-desktop.png">لقطة ديسكتوب عربي</a> · <a href="public-footer-responsive-2026-09-06/PUB-01-ar-tablet.png">لقطة تابلت عربي</a> · <a href="public-footer-responsive-2026-09-06/PUB-01-ar-mobile.png">لقطة موبايل عربي</a></p><p class="warn">هذا يثبت سلامة التشغيل والريسبونسيف، لكنه لا يمنح ${publicRows.length} صفحة Public نسبة Figma كاملة قبل القياس البصري المنضبط.</p></section>
...
<section><h2>تحديث PUB-04 — مقارنة الوحدات</h2><p class="ok"><strong>أُغلق نقص شريط المقارنة في الديسكتوب والموبايل.</strong></p><p>أصبح الشريط ثابتًا أسفل الشاشة ويعرض العقارين وصورهما، إزالة مستقلة، زر «قارن الآن»، ومسحًا محليًا مختصرًا على الموبايل. نجحت 4/4 حالات AR/EN مع عدم وجود تداخل أو خروج أفقي، و7/7 اختبارات مكوّن والبناء الكامل.</p><p><a href="public-comparison-tray-2026-09-06/results.json">نتائج الفحص</a> · <a href="public-comparison-tray-2026-09-06/ar-desktop.png">لقطة الديسكتوب</a> · <a href="public-comparison-tray-2026-09-06/ar-mobile.png">لقطة الموبايل</a></p></section>
